...
const blankForm = { name: "", name_ar: "", description: "", is_active: true };
...
export default function DepartmentFormPage() {
  ...
  useEffect(() => {
    ...
    getDepartments()
      .then((res) => {
        if (!res.success) throw new Error(res.message || t("organization.loadError"));
        const department = (res.data || []).find((item) => String(item.id) === String(id));
        if (!department) throw new Error(t("detail.notFound"));
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = isEditing ? await updateDepartment(id, payload) : await createDepartment(payload);
      if (!res.success) {
        setErrors(res.errors || {});
        throw new Error(res.message || t("organization.reviewData"));
      }

      showToast(isEditing ? t("organization.departmentUpdated") : t("organization.departmentCreated"));
    ...
    <div className="organization-page" style={{ ...s.page, direction: dir }} dir={dir}>
      ...
      <main className="organization-main" style={s.main}>
        ...
        <section className="organization-form-card" style={s.card}>
          <h1 style={s.title}>{isEditing ? t("organization.editDepartment") : t("organization.newDepartment")}</h1>
            ...
            <form onSubmit={handleSubmit} style={s.form}>
              ...
              <div className="organization-form-actions" style={s.actions}>
                <button type="submit" style={s.primaryBtn} disabled={saving}>
                  {saving ? t("common.saving") : t("organization.saveDepartment")}
                </button>
